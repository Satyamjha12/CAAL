...
const GITHUB_RAW_URL =
  'https://raw.githubusercontent.com/CoreWorxLab/caal-tools/main/docs/index.json';
...
export async function GET() {
  ...
  try {
    ...
    if (useLocal) {
      ...
      if (!res.ok) {
        console.error('[/api/tools/registry] GitHub fetch failed:', res.status);
        return NextResponse.json(
    ...
    return NextResponse.json({ tools: data, source: useLocal ? 'local' : 'github' });
  } catch (error) {
